...
public class HashMap<Key, Value> : IEnumerable<Pair<Key, Value>>
{
    ...
    public bool PutIfAbsent(Key key, Value value)
    {
        ...
        if (_values[hashValue] == null)
            _values[hashValue] = new List<Pair<Key, Value>>();
        _values[hashValue].Add(new Pair<Key, Value> { Key = key, Value = value });
        _keys.Add(key);
        return true;
    }
    //Ćwiczenie 3
    ...
    public bool Replace(Key key, Value value)
    {
        if (!_keys.Contains(key))
            return false;
        int hashValue = HashCode(key);
        foreach (var pair in _values[hashValue])
        {
...
//Ćwiczenie 5
//Nadpisz metodę GetHashCode i Equals w klasie Product
...
public class Product
{
    ...
    public override int GetHashCode()
    {
        ...
        hashValue += Price.GetHashCode();
        hashValue += Quantity.GetHashCode();
        hashValue += Unit.GetHashCode();
    ...
    public override bool Equals(Object other)
    {
        ...
        return Equals((Product)other); // Stack Overflow
    }
}
